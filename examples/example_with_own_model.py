...
bot.invite_tracker = InviteTracker(
    bot=bot,
    db_url="sqlite://invitetracker.sqlite3",
    initialize_db=False,  # do not initialize the database during startup because we are using a custom model, see below
)
...
async def on_connect() -> None:
    await Tortoise.init(
        db_url="sqlite://invitetracker.sqlite3",
        modules={
            "models": ["__main__", "disnake.ext.invitetracker.database.models.__init__"]
        },
    )
    await Tortoise.generate_schemas()
    logger.info("[+] Database initialized")
    # await bot.invite_tracker._load_database()
